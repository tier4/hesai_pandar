...
PointcloudXYZIRADT Pandar40Decoder::convert(size_t block_id)
{
  ...
  for (auto unit_id : firing_order_) {
    ...
    //calc index from deg
    float azimuth_index_f = (azimuth_offset_[unit_id] + (static_cast<double>(block.azimuth)) / 100.0) / ROTATION_RESOLUTION;
    uint32_t abs_mod_azimuth_index_f = (uint32_t)abs(round(azimuth_index_f)) % ROTATION_MAX_UNITS;
    ...
    float elev_index_f = elev_angle_[unit_id] / ROTATION_RESOLUTION;
    uint32_t abs_mod_elev_index_f = (uint32_t)abs(round(elev_index_f)) % ROTATION_MAX_UNITS;
