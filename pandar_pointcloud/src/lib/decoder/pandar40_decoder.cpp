...
Pandar40Decoder::Pandar40Decoder(Calibration& calibration, float scan_phase, ReturnMode return_mode)
{
  ...
  for (size_t laser = 0; laser < LASER_COUNT; ++laser) {
    elev_angle_[laser] = (int32_t)round(calibration.elev_angle_map[laser] / ROTATION_RESOLUTION);
    azimuth_offset_[laser] = (int32_t)round(calibration.azimuth_offset_map[laser] / ROTATION_RESOLUTION);
  }
  ...
  for (uint32_t rot_index = 0; rot_index < ROTATION_MAX_UNITS; ++rot_index) {
    double rotation = deg2rad(ROTATION_RESOLUTION * rot_index);
...
PointcloudXYZIRADT Pandar40Decoder::convert(size_t block_id)
{
  ...
  for (auto unit_id : firing_order_) {
    ...
    //calc index from deg
    //(block.azimuth / 100) * ROTATION_RESOLUTION 
    int32_t azimuth_index_f = (azimuth_offset_[unit_id] + block.azimuth * 10);
    uint32_t abs_mod_azimuth_index_f = (uint32_t)abs(azimuth_index_f) % ROTATION_MAX_UNITS;
    ...
    if(abs_mod_azimuth_index_f == 0) {
      ...
      if(azimuth_index_f < 0) {
        // minus deg pattern
        azimuth_index = ROTATION_MAX_UNITS - abs_mod_azimuth_index_f;
      }
      else {
        // plus deg pattern
    ...
    uint32_t abs_mod_elev_index_f = (uint32_t)abs(elev_angle_[unit_id]) % ROTATION_MAX_UNITS;
    ...
    if(abs_mod_elev_index_f == 0) {
      ...
      if(elev_angle_[unit_id] < 0) {
        // minus deg pattern
        elev_index = ROTATION_MAX_UNITS - abs_mod_elev_index_f;
      }
      else {
        // plus deg pattern
